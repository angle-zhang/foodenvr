...
la_cb <- get_census_blocks(path = base_path, crs = proj_crs, state = proj_state,
                            year = proj_year, county = proj_county)
...
save_and_clean_poi(year = proj_year, state = proj_state,
                   processed_path = processed_path, boundary = buffer)

foodpoi <- get_foodpoi(year = proj_year, path = processed_path) %>%
